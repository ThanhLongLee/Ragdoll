...
<%@ Import Namespace="Unity.Common.Configuration" %>
<%@ Import Namespace="Unity.Common.Helper" %>
...
<script runat="server">
    ...
    public override bool CheckAuthentication()
    {
        ...
        //Session["UserCode"] = "GomiStore001" + (int)AccountType.Store;
        //Session["UserCode"] = "GomiStore002" + (int)AccountType.Store;
        ...
        //return Session["AccountRole"] != null;
        return true;
    ...
    public override void SetConfig()
    {
        ...
        LicenseName = "gomdori";
        LicenseKey = "RLSB97455QNPBBQELHUSYH8A3GEFFAXQ";


        var folderTarget = string.Empty;
        //var userRole = AccountRole.Undefined;
        var userRole = AccountRole.Admin;

        //if (Session["AccountRole"] != null)
        //{
        //    userRole = (AccountRole)Convert.ToInt32(Session["AccountRole"].ToString().Split(AppSettings.Delimiter).First());
        //    folderTarget = Session["AccountRole"].ToString().Split(AppSettings.Delimiter).Last();
        //}

        if (userRole == AccountRole.Admin)
        {
            folderTarget = AppSettings.ContentPath;
        }

        // The base URL used to reach files in CKFinder through the browser.
        //BaseUrl =AppSettings.ImageHosting + folderTarget;
        BaseUrl = AppSettings.ImageHosting + AppSettings.ContentPath;

        // The phisical directory in the server where the file will end up. If
        // blank, CKFinder attempts to resolve BaseUrl.
        BaseDir = FileHelper.ResourcesPath(folderTarget).Replace("\\", "/");
        ...
        Thumbnails.Dir = userRole == AccountRole.Admin ? FileHelper.ResourcesPath("").Replace("\\", "/") : FileHelper.ResourcesPath(AppSettings.ContentPath).Replace("\\", "/");

        Thumbnails.Enabled = true;
        Thumbnails.DirectAccess = false;
        Thumbnails.MaxWidth = 0;
        Thumbnails.MaxHeight = 0;
        ...
        RoleSessionVar = "CKFinder_AccountRole";
        ...
        if (userRole == AccountRole.Undefined)
        {
        ...
        {
            AccessControl aclUser = AccessControl.Add();
            aclUser.Role = "*";
            aclUser.ResourceType = "*";
            aclUser.Folder = "/";

            aclUser.FolderView = true;
            aclUser.FolderCreate = true;
            aclUser.FolderRename = true;
            aclUser.FolderDelete = true;

            aclUser.FileView = true;
            aclUser.FileUpload = true;
            aclUser.FileRename = true;
            aclUser.FileDelete = true;
        }
        ...
        if (userRole != AccountRole.Admin)
        {
            type = ResourceType.Add(folderTarget.Split('/')[folderTarget.Split('/').Length - 2]); // ten thu muc hien thi
            type.Url = BaseUrl;
            type.Dir = BaseDir;
            type.MaxSize = 0;
        ...
        {
            type = ResourceType.Add(AppSettings.ContentPath.Split('/').First()); // ten thu muc hien thi
            type.Url = BaseUrl;
            type.Dir = BaseDir;
            type.MaxSize = 0;
            type.DeniedExtensions = new string[] { };
